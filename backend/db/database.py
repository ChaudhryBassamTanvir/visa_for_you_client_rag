...
class ChatMessage(Base):
    ...
    id         = Column(Integer, primary_key=True, autoincrement=True)
    user_id    = Column(Integer, ForeignKey("users.id"), nullable=True)   # ✅ FK restored
    role       = Column(String(10))
    content    = Column(Text)
    created_at = Column(DateTime, default=datetime.utcnow)
    user       = relationship("User", back_populates="messages")          # ✅ back-ref added
...
def get_all_clients():
    ...
    try:
        ...
        # Also include web users (non-admin) as clients
...
def save_chat_message(user_id: int, role: str, content):
    """
    Accepts content as a string OR as an Anthropic API response list.
    Always stores a plain string in the DB.
    """
    # ✅ Fix: extract text if content is a list of blocks (raw Anthropic response)
